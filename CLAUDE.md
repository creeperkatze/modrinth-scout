...
- `src/db/`: Mongoose schemas and queries (`src/db/queries.ts`) for the unified `tracking` collection (`schemas/tracking.ts`), server config, and donators (`schemas/donator.ts`)
