# Terms of Service

By using Modrinth Scout ("the bot"), you agree to these terms.

## Using the bot

The bot is provided as-is, with no uptime guarantee. It may be unavailable, changed, or discontinued at any time. Data shown by the bot comes from the Modrinth API and may occasionally be inaccurate or outdated.

## Acceptable use

Don't use the bot to spam, abuse, or disrupt Discord servers, or to break Discord's [Terms of Service](https://discord.com/terms) or [Community Guidelines](https://discord.com/guidelines). Servers or users that abuse the bot may be blocked from using it.

## Donator perks

Donating via Ko-fi is optional and supports development. Donations are voluntary and non-refundable.

A donation activates perks for a single Discord server and cannot be transferred or reused on another server. Which perks are included, and any limits attached to them, may change over time.

## Liability

The bot is not affiliated with Modrinth or Discord and is provided without warranty. Use it at your own risk.

## Changes

These terms may change over time. Continuing to use the bot after a change means you accept the updated terms.

## Contact

contact@creeperkatze.dev
